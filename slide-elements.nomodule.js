 ...
 *     cache.
 */
function render$1(result, container, templateFactory = defaultTemplateFactory) {
...
function render$$1(result, container, scopeName) {
    return render$1(result, container, shadyTemplateFactory(scopeName));
}
...
const e=Symbol("tag"); const s=Symbol("needsRender"); const i=t=>t.replace(/([a-z](?=[A-Z]))|([A-Z](?=[A-Z][a-z]))/g,"$1$2-").toLowerCase(); const o=t=>{t.$={}, t.shadowRoot.querySelectorAll("[id]").forEach(e=>{t.$[e.id]=e;});};class h extends HTMLElement{static get is(){return this.hasOwnProperty(e)&&this[e]||(this[e]=i(this.name))}connectedCallback(){"template"in this&&(this.attachShadow({mode:"open"}), this.render({sync:!0}), o(this));}async render({sync:e=!1}={}){this[s]=!0, e||await 0, this[s]&&(this[s]=!1, render$$1(this.template,this.shadowRoot,this.constructor.is));}}

let e$1=!1;const n=[]; const r=t=>{e$1||(window.addEventListener("hashchange",c), window.addEventListener("location-changed",c), window.addEventListener("popstate",c), e$1=!0), n.push(t);}; const c=()=>{n.forEach(e=>e(d(),p(),h$1()));}; const d=()=>window.decodeURIComponent(window.location.pathname); const p=()=>window.location.search.slice(1); const h$1=()=>window.decodeURIComponent(window.location.hash.slice(1));
...
class SlidemDeck extends h {
  ...
  connectedCallback() {
    ...
    this.$.presenterToggle.addEventListener('click', () => {
      ...
      changeLocation({ query: (this.presenter && 'presenter') || '', hash: h$1() });
    });
    ...
    const changeLocation = ({ path = d(), query = p(), hash = h$1() } = {}) => {
...
class SlidemSlideBase extends h {
  get template() {
    if (this.getAttribute('fullscreen') !== null || this.constructor.fullscreen) {
      return html$1`
        ${slidemStyle}
        ${(this.constructor.name !== 'SlidemSlide' && this.content) || html$1`<slot id="slot"></slot>`}
      `;
    } else {
      return html$1`
        ${slidemStyle}
